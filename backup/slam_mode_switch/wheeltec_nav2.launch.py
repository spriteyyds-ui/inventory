...
def generate_launch_description():
    ...
    # slam_mode: none=使用AMCL定位 | localization=SLAM Toolbox纯定位 | lifelong=SLAM Toolbox动态建图
    # 当 slam_mode 不为 none 时，禁用 AMCL 和 map_server，由 SLAM Toolbox 提供定位和地图
    slam_mode_arg = DeclareLaunchArgument(
        'slam_mode',
        default_value='none',
        description='SLAM模式: none(使用AMCL) | localization(SLAM纯定位) | lifelong(SLAM动态建图)')

    # 当 slam_mode 不为 none 时，传 slam=True 给 bringup 以禁用 AMCL + map_server
    use_slam = LaunchConfiguration('slam_mode') != 'none'
