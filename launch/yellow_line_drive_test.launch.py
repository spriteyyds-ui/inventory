...
def generate_launch_description():
    ...
    target_x_ratio_arg = DeclareLaunchArgument(
        'yellow_line_target_x_ratio', default_value='0.924',
        description='黄线目标 x 位置比例')
    linear_speed_arg = DeclareLaunchArgument(
        'test_linear_speed', default_value='0.20',
        description='前进线速度 m/s')
    backward_linear_speed_arg = DeclareLaunchArgument(
        'test_backward_linear_speed', default_value='0.20',
        description='后退线速度 m/s；与 rear_target_backup_speed 一致')
